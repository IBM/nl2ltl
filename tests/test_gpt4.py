...
from nl2ltl.engines.gpt4.core import GPT4Engine
...
class TestGPT4:
    ...
    def test_gpt4_engine_basic(self, utterance):
        """Test GPT-3 engine for utterances with basic filter."""
        output = translate(utterance, self.gpt4_engine, self.basic_filter)
        assert isinstance(output, Dict)

    @pytest.mark.parametrize("utterance", UtterancesFixtures.utterances)
    def test_gpt4_engine_greedy(self, utterance):
        """Test GPT-3 engine for utterances with greedy filter."""
        output = translate(utterance, self.gpt4_engine, self.greedy_filter)
        assert isinstance(output, Dict)
